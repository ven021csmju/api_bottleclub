...
def upgrade() -> None:
    ...
    op.create_index(
        "uq_product_org_barcode", "products",
    ...
    # 17. inventory_lots (FK → purchase_receivings, stock_transfers — both exist now)
    op.create_table(
        "inventory_lots",
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column("branch_id", sa.BigInteger(), nullable=False),
        ...
        sa.ForeignKeyConstraint(["branch_id"], ["branches.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["product_id"], ["products.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["purchase_receiving_id"], ["purchase_receivings.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["stock_transfer_id"], ["stock_transfers.id"], ondelete="SET NULL"),
    ...
    op.create_index(
        "ix_inventory_lot_fefo", "inventory_lots",
        ["branch_id", "product_id", "expiry_date"],
    )
    ...
    op.create_index(
        "ix_inventory_low_stock", "inventory",
        ["branch_id", "product_id"],
        postgresql_where="on_hand <= 10",
    )

    # 19. stock_transfer_items (FK → inventory_lots)
    op.create_table(
    ...
    # 20. stock_movements (FK → inventory_lots)
    op.create_table(
        "stock_movements",
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column("branch_id", sa.BigInteger(), nullable=False),
    ...
    op.create_index(
        "uq_customer_org_phone", "customers",
    ...
    op.create_index(
        "ix_loyalty_txn_customer_expires", "loyalty_transactions",
    ...
    op.create_table(
        ...
        sa.Column("branch_ids", postgresql.ARRAY(sa.Integer()), nullable=True),
    ...
    op.create_index(
        "ix_promotions_active", "promotions",
    ...
    op.create_index(
        "uq_orders_idempotency_key", "orders",
        ["idempotency_key"],
        unique=True,
    ...
    # Now add the circular FK: returns.refund_id → refunds.id
    op.create_foreign_key(
        "fk_returns_refund_id", "returns", "refunds", ["refund_id"], ["id"], ondelete="SET NULL",
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("coupon_id", "customer_id", name="uq_coupon_usage"),
    )
    ...
    op.create_index(
        "ix_refresh_tokens_valid", "refresh_tokens",
        ["user_id", "expires_at"],
    ...
    # 40. document_sequences (composite PK, no FKs)
    op.create_table(
        "document_sequences",
