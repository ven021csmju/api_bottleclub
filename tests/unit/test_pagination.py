...
class TestPaginate:
    ...
    def test_returns_tuple_of_four(self):
        ...
        result = paginate(session, MagicMock(), page=1, per_page=20)
        assert isinstance(result, tuple)
        assert len(result) == 4

    def test_pages_zero_when_no_items(self):
        session = self._make_mock_session(total=0)
        _, total, page, pages = paginate(session, MagicMock(), page=1, per_page=20)
        assert total == 0
        assert pages == 0

    def test_pages_calculation(self):
        session = self._make_mock_session(total=45)
        _, total, _, pages = paginate(session, MagicMock(), page=1, per_page=20)
        assert total == 45
        assert pages == 3  # ceil(45/20)

    def test_pages_exact_multiple(self):
        session = self._make_mock_session(total=40)
        _, _, _, pages = paginate(session, MagicMock(), page=1, per_page=20)
        assert pages == 2
